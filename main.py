...
for docx_file in os.listdir(DOCX_DIR):
    ...
    if docx_file != ".gitignore":
        ...
        prefix = f"{docx_file[0:3].upper()}_{docx_file[7:9]}_Q_"
        ...
        for i, e in enumerate(content):
            if not (e.startswith(MULTIPLE_CHOICE) or e.startswith(TRUE_FALSE) or e.startswith("DOM")):
        ...
        text.appendChild(dom.createTextNode(f"$course$/top/{docx_file[0:9]}"))
